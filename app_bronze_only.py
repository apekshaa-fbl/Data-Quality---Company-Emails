...
# Header (markdown link for reliable redirect)
...
# ============================================================
# Persistence (run logs, metrics, comments)
# ============================================================
...
# Composite score for one end-of-release trend bar
# Higher is better. For "bad" metrics we convert to (100 - bad_pct).
...
# ============================================================
# Tabs: Checks vs Dashboard (separate page feel)
# ============================================================
...
with checks_tab:
    ...
    with run_col2:
        st.button("Go to Dashboard ➜")  # UX hint; tabs are above
    with run_col3:
        st.caption("Tip: Turn on persistence to compare releases over time (one bar per release in the Trends chart).")
    ...
    with email_per_company as (
      ...
      from {BRZ_DOMAINS} as d
      left join {BRZ_EMAILS} as e
        on d.id = e.id
    ...
    """
    with st.expander("SQL Query (Coverage)", expanded=False):
        st.code(COVERAGE_SQL, language="sql")
    ...
    st.info("Coverage is computed using Bronze domains as the company universe and joining Bronze emails by id.")
    ...
    st.info("High not-200 indicates domain quality issues or enrichment gaps. Missing status indicates reachability checks not run/landed.")
    ...
    """
    with st.expander("SQL Query (Format)", expanded=False):
    ...
    st.info("Invalid formats often come from masked emails, placeholders, or parsing artefacts.")
    ...
    FROM summary;
    """
    with st.expander("SQL Query (Duplicates)", expanded=False):
    ...
    st.info("Duplicates inflate coverage. Enforce dedupe and identify upstream causes.")
    ...
    DOMAIN_MATCH_SQL = f"""
    WITH base AS (
        SELECT d.id, lower(trim(coalesce(d.fqdn, ''))) AS fqdn, lower(trim(coalesce(e.email, ''))) AS email
        FROM {BRZ_DOMAINS} d
        LEFT JOIN {BRZ_EMAILS} e ON d.id = e.id
    ),
    parsed AS (
        SELECT *, split_part(fqdn, '.', 1) AS company_root, split_part(split_part(email, '@', 2), '.', 1) AS email_root
        FROM base
    ...
    FROM parsed;
    """
    with st.expander("SQL Query (Domain match)", expanded=False):
        st.code(DOMAIN_MATCH_SQL, language="sql")
    ...
    st.info("Mismatch may be valid (free-mail/SMBs/group domains) or may indicate mapping/parsing issues.")
    ...
    classified as (
      select *, case when email_clean ~ '^(no-reply|noreply|do-not-reply|donotreply)@' then 1 else 0 end as is_blocked
      from valid
    )
    select round(100.0 * sum(is_blocked) / nullif(count(*),0), 2) as pct_blocked
    from classified;
    """
    with st.expander("SQL Query (Blocked)", expanded=False):
    ...
    st.warning("Blocked/no-reply emails should be removed immediately.")
    ...
    st.info("Track NULL vs verified/failed shares over releases to ensure verification coverage improves.")
    ...
    st.info("Use this metric to enforce a strict top-5 policy and reduce noise.")
    section_comment_ui("gt5_emails")

    st.divider()

    # 9) Priority rules
    st.header("9) Priority selection rules (Top 5 per company/country)")
    st.markdown("""
...
with dashboard_tab:
    ...
    if not st.session_state.metrics_for_dashboard:
        st.info("Run the checks in the **✅ Checks (Bronze)** tab to populate this dashboard.")
    else:
        ...
        if not persist_to_db:
            ...
            # Save composite for current run
            ...
            if trend.empty:
                ...
                st.caption(
                    "Composite score is calculated from key metrics. Bad metrics are inverted as (100 - pct), "
                    "so the final score is always 'higher is better'. This gives you **one bar per release**."
                )
